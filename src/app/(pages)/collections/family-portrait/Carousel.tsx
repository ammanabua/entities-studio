...
const Carousel = ({ collection }: CarouselProps) => {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1 === collection.length ? 0 : prevIndex + 1))
  }
  const handlePrevious = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 < 0 ? collection.length - 1 : prevIndex - 1))
  }
    ...
    <>
      <div key={collection[currentIndex]} className="flex w-full justify-center items-center lg:items-start">
        <AnimatePresence mode="wait">
          ...
          >
            <h3 className="text-xl md:text-4xl md:font-bold text-left tracking-wide mb-8">
              {collection[currentIndex].title}
            </h3>
            <p className="font-rubik text-left text-xl font-light tracking-wider mb-12 w-full">
              {collection[currentIndex].desc}
            </p>
          </motion.div>
        </AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Link href={`/images/${collection[currentIndex]._id}`}>
              <Image src={collection[currentIndex].url} alt="Voids" width={490} height={0} sizes="100vw" className="w-full lg:h-96" />
            </Link>

            <div className="lg:hidden flex w-28 align-center justify-between mt-12 font-rubik">
              <button className="w-12 h-12 bg-white items-center flex align-center justify-center font-light hover:bg-[#EB6F07] hover:text-white" onClick={handlePrevious}>
                ‹
              </button>
              <button className="w-12 h-12 bg-white items-center flex align-center justify-center font-light hover:bg-[#EB6F07] hover:text-white" onClick={handleNext}>
                ›
              </button>
            </div>
          </motion.div>
      ...
      <div className="hidden lg:flex w-2/3 lg:ml-60 items-center justify-between bottom-4 font-rubik z-40 text-[#777D96] absolute px-16">
        ...
        <div className="font-rubik flex items-center">
          <span className="text-[40px] text-[#32374B]">{currentIndex <= 8 ? '0' : ''}{currentIndex + 1}</span>
